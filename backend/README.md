...
```bash
cd backend
cp .env.example .env          # fill the credentials for the source(s) you're wiring
npm install
...
npm run dev
```
The server is **live-only**: each route serves real data once that source is connected, otherwise it
returns 401 (Microsoft) / 503 (QuickBooks). There is no runtime sample mode — the test gate exercises
the same live route + transforms path against synthetic upstream fixtures (`FIXTURES_DIR`, see
`../docs/TESTING.md`).

## Go live on a source (sandbox or production)
1. Fill the credentials for the source(s) you're wiring:
...
2. In `frontend/.env`, set `VITE_API_URL` to this backend so the SPA fetches live data.
