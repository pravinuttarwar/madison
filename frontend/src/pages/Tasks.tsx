...
import { getTasks, sourceModeFor, type OwnerTasks, type TasksData } from '@/lib/api';
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Open To Do items across the team — {owners.length} people, sorted by overdue.
          </p>
        </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-dashed border-border bg-muted/20 p-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">
            ...
            configured (read-only, via the granted Tasks permission).
          </p>
        </div>
      </div>
    </div>
...
export default function Tasks() {
  const { data, loading, error } = useApi<TasksData>(getTasks, []);
  ...
  const myTasks = isLive ? all : all.filter((t) => t.owner === 'DCR');
